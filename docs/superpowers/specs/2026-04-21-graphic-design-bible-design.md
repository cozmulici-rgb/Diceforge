...
Diceforge has coherent visual intent embodied in three places — the `FacetboundTheme` runtime theme, the start-menu art pack in `game/assets/start-menu/exports/`, and the media-design brief in `docs/archive/design/game-development-phases/game-design.md` §15 — but no single document an artist or illustrator can read cold and produce against.
...
- The Facetbound visual language is encoded in GDScript constants, not in prose; production decisions cannot be made without reading code.
...
- Distills the shipped Facetbound style from the theme code, shipped art pack, and current `.tscn` compositions into a designer-readable reference.
...
- Not a developer style guide — component wiring and theme-code details live in the `FacetboundTheme` source.
- Not a creative re-brief — the shipped Facetbound look is the reference, not a proposal to replace.
...
├── facetbound-style-guide.md        # foundation: palette, type, motifs, components, voice
...
## 6. Foundation Doc — `facetbound-style-guide.md`
...
3. **Palette.** Five functional bands: void background, panel neutrals, gold accent, cyan accent, ember red. Each entry: display name, hex, role, `FacetboundTheme` constant it maps to (e.g. `ACCENT_GOLD = #c6a85a`), and a "never use for" line. Values match the current `FacetboundTheme` source.
...
10. **Appendix A — Theme constant map.** Compact table: every named visual decision (color, type role, component, stylebox) → the `FacetboundTheme` constant or class it is bound to. Keeps the bible honest to the engine.
...
- "Need palette, type, or a specific motif" → `facetbound-style-guide.md`.
- "Need to know what's missing" → `asset-production.md` (Asset backlog).
- "Need the theme constant behind a color" → `facetbound-style-guide.md` Appendix A.
...
- **Palette values** come from `game/scripts/ui/facetbound_theme.gd` constants. If the bible says a color, it must be verifiable against that file.
...
- An illustrator who has never opened the repo can read `facetbound-style-guide.md` + one `screen-briefs/<screen>.md` + `asset-production.md` and produce art that looks like it belongs in Diceforge.
- Every color, type role, and component referenced in the bible maps to a real `FacetboundTheme` constant or a real shipped asset.
...
- **Drift risk.** The bible can fall out of sync with `FacetboundTheme` if the theme changes. Mitigation: Appendix A is the single cross-check surface; include a note that it must be updated when the theme file changes.
